...
type WithdrawRequest struct {
	...
	BankCardID   uint           `gorm:"not null"`                                 // 银行卡ID
	Amount       float64        `gorm:"type:decimal(15,2);not null"`              // 提现金额
	Fee          float64        `gorm:"type:decimal(15,2);default:0"`             // 手续费
	ActualAmount float64        `gorm:"type:decimal(15,2);not null"`              // 实际到账
	...
	VoucherURL   string         `gorm:"type:varchar(500)"`                        // 打款凭证
	ReviewedAt   *time.Time     // 审核时间
	PaidAt       *time.Time     // 打款时间
